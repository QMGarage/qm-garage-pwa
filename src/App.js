...
<meta name="apple-mobile-web-app-title" content="QM Garage"/>
<title>QM Garage</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/react/18.2.0/umd/react.production.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/react-dom/18.2.0/umd/react-dom.production.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/babel-standalone/7.23.2/babel.min.js"></script>
<style>
*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;margin:0;padding:0;}
body{font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',Arial,sans-serif;background:#eef1f5;overscroll-behavior:none;}
...
const ORANGE="#ff5a00",DARK="#0d0d0d",GRAY="#f4f5f7",BORDER="#dde0e5";
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:4,flex:half?"0 0 calc(50% - 6px)":"1 1 100%"}}>
      ...
      <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
        style={{padding:"10px 12px",border:`1.5px solid ${BORDER}`,borderRadius:8,fontSize:14,background:"#fff",outline:"none",width:"100%",color:DARK,boxSizing:"border-box"}}
      ...
      <div style={{display:"flex",gap:6}}>
        {options.map(opt=>(
          <label key={opt.val} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,cursor:"pointer"}}>
            <input type="radio" name={name} value={opt.val} checked={value===opt.val} onChange={()=>onChange(opt.val)}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <textarea value={data.motif} onChange={e=>set("motif",e.target.value)} placeholder="Décrivez le motif de la visite..."
        style={{width:"100%",minHeight:80,padding:"10px 12px",border:`1.5px solid ${BORDER}`,borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box",color:DARK,background:"#fff"}}
        onFocus={e=>e.target.style.borderColor=ORANGE} onBlur={e=>e.target.style.borderColor=BORDER}/>
    </div>
  );
}
        ...
        {positions.map(([key,label])=>(
          <div key={key} style={{display:"grid",gridTemplateColumns:"1fr 90px 90px",padding:"6px 10px",borderBottom:`1px solid ${BORDER}`,alignItems:"center"}}>
            ...
            <input value={data[key].usure} onChange={e=>set(key,{...data[key],usure:e.target.value})} placeholder="ex: 4"
              style={{textAlign:"center",border:`1px solid ${BORDER}`,borderRadius:6,padding:"5px",fontSize:13,width:"100%",boxSizing:"border-box"}}/>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <textarea value={data.travaux} onChange={e=>set("travaux",e.target.value)} placeholder="Rien à signaler..."
        style={{width:"100%",minHeight:70,padding:"10px 12px",border:`1.5px solid ${BORDER}`,borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box",color:DARK,background:"#fff"}}
        onFocus={e=>e.target.style.borderColor=ORANGE} onBlur={e=>e.target.style.borderColor=BORDER}/>
...
/* ENTRETIEN : pleine largeur, 3 colonnes internes */
...
.grp-head{font-size:8px;font-weight:700;text-transform:uppercase;color:#fff;padding:3px 6px;background:#333;letter-spacing:0.5px;border-bottom:1px solid #555;}
...
/* LIGNE 2 : Pneus + Freinage côte à côte */
.row2{display:grid;grid-template-columns:1fr 1fr;gap:7px;}

/* LIGNE 3 : Fluides + Éclairage + Sous-véhicule */
...
<!-- ENTRETIEN PLEINE LARGEUR 3 COLONNES -->
<div>
  <div class="sec">🛠️ Entretien & Filtres</div>
  <div class="entretien-wrap">
    <div class="entretien-cols">
      ${(()=>{
        // Répartir les 6 groupes sur 3 colonnes (2 groupes par colonne)
        const cols=[[entretienGroups[0],entretienGroups[1]],[entretienGroups[2],entretienGroups[3]],[entretienGroups[4],entretienGroups[5]]];
        return cols.map(colGroups=>`
          <div class="entretien-col">
            ${colGroups.map(g=>`
              <div class="grp-head">${g.title}</div>
...
function App(){
  ...
  const setField=(section,key,val)=>{
    if(section==="root"){setData(d=>({...d,[key]:val}));}
    else{setData(d=>({...d,[section]:{...d[section],[key]:val}}));}
  };
  ...
  const generateAndDownload=async()=>{
    ...
    try{
      ...
      // Ouvre dans un nouvel onglet et déclenche l'impression directement
      const printWin=window.open('','_blank');
      if(printWin){
        printWin.document.open();
        printWin.document.write(html);
        printWin.document.close();
        // Attend que le contenu soit chargé puis imprime
        printWin.onload=()=>{
          setTimeout(()=>{
            printWin.focus();
            printWin.print();
        ...
        // Fallback si popup bloquée : remplace la page courante
  ...
  return(
    <div style={{minHeight:"100vh",background:"#eef1f5"}}>
      ...
      <div style={{background:DARK,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 8px rgba(0,0,0,0.3)"}}>
        <div style={{padding:"10px 16px 0"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`2px solid ${ORANGE}`,paddingBottom:8,marginBottom:8}}>
            <div>
              <div style={{fontSize:22,fontWeight:900,fontStyle:"italic",color:"#fff",letterSpacing:-1}}>QM <span style={{color:ORANGE}}>GARAGE</span></div>
            ...
            <div style={{border:`1.5px solid ${ORANGE}`,borderRadius:4,padding:"2px 6px",display:"flex",alignItems:"center",gap:4}}>
              ...
              <input
                value={data.numFiche}
                onChange={e=>setField("root","numFiche",e.target.value)}
                style={{width:58,background:"transparent",border:"none",outline:"none",color:ORANGE,fontWeight:700,fontSize:13,textAlign:"center",padding:0}}
                maxLength={10}
              />
            </div>
          </div>
        ...
        <div style={{display:"flex",overflowX:"auto",padding:"0 8px",gap:4,scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
          {STEPS.map((s,i)=>(
      ...
      {/* BARRE BAS */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#fff",borderTop:`1px solid ${BORDER}`,padding:"10px 16px",display:"flex",gap:10,alignItems:"center",boxShadow:"0 -2px 8px rgba(0,0,0,0.08)",paddingBottom:`calc(10px + env(safe-area-inset-bottom))`}}>
        {step>0&&(
          <button onClick={()=>setStep(s=>s-1)} style={{flex:1,padding:"12px",background:GRAY,border:`1px solid ${BORDER}`,borderRadius:10,fontSize:14,fontWeight:700,color:DARK,cursor:"pointer"}}>← Retour</button>
        )}
        {step<STEPS.length-1?(
          ...
          <>
            <button onClick={sendByMail} style={{flex:1,padding:"12px",background:"#1a1a2e",border:"none",borderRadius:10,fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer"}}>📧 Email</button>
            <button onClick={generateAndDownload} disabled={generating} style={{flex:2,padding:"12px",background:generating?"#ccc":ORANGE,border:"none",borderRadius:10,fontSize:14,fontWeight:700,color:"#fff",cursor:generating?"not-allowed":"pointer"}}>{generating?"⏳ Génération...":"🖨️ Exporter PDF"}</button>
          </>
        )}
      </div>
      {/* TOAST */}
      {toast&&(
        <div style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",background:toast.type==="error"?"#c0392b":DARK,color:"#fff",padding:"10px 20px",borderRadius:10,fontSize:13,fontWeight:600,zIndex:9999,boxShadow:"0 4px 16px rgba(0,0,0,0.3)",maxWidth:"90vw",textAlign:"center"}}>{toast.msg}</div>
      )}
    </div>
  );
}
